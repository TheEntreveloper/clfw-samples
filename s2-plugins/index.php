...
// in this sample we will only add pages
$app
    // in this sample we extend the framework with a Plugin, which we will use to implement the same search
    // functionality we have in a page in Sample 1 (s1).
    // As with pages, we first define a key for the plugin (can be a string, a regular expression or an array of strings
    // or regular expressions). The key defines what events or request keys the Plugin will respond to.
    // The 2nd parameter is the name of the Plugin (the name of the Plugin class and the Plugin File)
    // The framework finds Plugin using a convention. By default, it looks for Plugins in the plugin folder of the App.
    ...
    ->addPage(['about'], ['parts/header', 'pages/about', 'parts/footer'], array(
        'title' => 'About page for Codelib sample S2'), 'none', true)
    ...
    // opening this page (clkey=links) will cause the error page to display because pages/wrongpage.php does not exist
    ->addPage(['links'], ['parts/header', 'pages/wrongpage', 'parts/footer'], array(
        'title' => 'Contacting me'
    ), 'none')
    // notice how access to 'mypage' is denied by default if you try viewing it on your web browser. All it takes to
    // have that protection is adding the 'LoggedIn' value to the 'protection' parameter of mypage.
    ->addPage(['mypage'], ['parts/header', 'pages/personal', 'parts/footer'], array(
        'title' => 'Contacting me'
    ), 'LoggedIn')
